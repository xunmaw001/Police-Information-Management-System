        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '人员名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.renliziyuanName" placeholder="人员名称" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('renliziyuan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jingwuxinxiguanli/upload/renliziyuanMuBan.xls"
                        >批量导入人力资源数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('renliziyuan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="renliziyuanUploadSuccess"
                                :on-error="renliziyuanUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('renliziyuan','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入人力资源数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('renliziyuan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "renliziyuan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="renliziyuanName"
                                   header-align="center"
                                   label="人员名称">
                        <template slot-scope="scope">
                            {{scope.row.renliziyuanName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="renliziyuanPhone"
                                   header-align="center"
                                   label="人员手机号">
                        <template slot-scope="scope">
                            {{scope.row.renliziyuanPhone}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="renliziyuanIdNumber"
                                   header-align="center"
                                   label="人员身份证号">
                        <template slot-scope="scope">
                            {{scope.row.renliziyuanIdNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="renliziyuanPhoto"
                                     header-align="center"
                                     width="200"
                                     label="人员头像">
                        <template slot-scope="scope">
                            <div v-if="scope.row.renliziyuanPhoto">
                                <img :src="$base.url+scope.row.renliziyuanPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="renliziyuanEmail"
                                   header-align="center"
                                   label="人员邮箱">
                        <template slot-scope="scope">
                            {{scope.row.renliziyuanEmail}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('renliziyuan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('renliziyuan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('renliziyuan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('renliziyuan','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                ...
                username : null,
                password : null,
                renliziyuanName : null,
                renliziyuanPhone : null,
                renliziyuanIdNumber : null,
                renliziyuanPhoto : null,
                sexTypes : null,
                renliziyuanEmail : null,
                renliziyuanDelete : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                ...
                //本表字段
                     '账户': "username",
                     '人员名称': "renliziyuanName",
                     '人员手机号': "renliziyuanPhone",
                     '人员身份证号': "renliziyuanIdNumber",
                     '人员头像': "renliziyuanPhoto",
                     '性别': "sexValue",
                     '人员邮箱': "renliziyuanEmail",
                     '添加时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'renliziyuan',//当前表表名,
                        sumColum : 'renliziyuan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'renliziyuan_name',//分组字符串字段
                        // types : 'renliziyuan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.renliziyuanName!= '' && this.searchForm.renliziyuanName!= undefined) {
                    params['renliziyuanName'] = '%' + this.searchForm.renliziyuanName + '%'
                }
                ...
                params['renliziyuanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "renliziyuan/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            resetPassword(id) {
                ...
                //重置密码
                this.$http({
                    url: "renliziyuan/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            renliziyuanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "renliziyuan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "renliziyuan/update",
                        method: "post",
                        data: {
                            id:id,
//                            renliziyuanTypes:renliziyuanTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
